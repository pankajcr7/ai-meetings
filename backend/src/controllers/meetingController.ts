...
import {
  ...
  extractActionItems,
  transcribeAudio,
  analyzeMeetingQuality,
  analyzeSentiment,
  extractDecisions,
  detectConflicts,
  generateVoiceSummaryText,
...
export const processMeeting = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Generate all unique AI features in parallel for efficiency
    try {
      const participants = meeting.participants.length > 0 ? meeting.participants : ['Speaker 1', 'Speaker 2'];
      const duration = meeting.duration || 1800; // Default 30 min
      ...
      const [
        ...
        // 1. Meeting Quality Score
        analyzeMeetingQuality(transcript, duration, actionItems.length, participants),
        // 2. Sentiment Timeline
        analyzeSentiment(transcript),
        // 3. Decision Registry
        extractDecisions(transcript),
        // 4. Conflict Detector
        detectConflicts(transcript),
        // 5. Voice Summary Text
        generateVoiceSummaryText(summary, actionItems.map(a => a.title)),
      ]);

      // 5. Meeting Cost Calculator
      const hourlyRate = 75; // Default average hourly rate
      const participantCount = Math.max(2, participants.length);
      const durationHours = duration / 3600;
      const meetingCost = {
        totalCost: hourlyRate * participantCount * durationHours,
        hourlyRate,
        participantCount,
        currency: 'USD',
      };

      // Save all new features to meeting
      meeting.quality = qualityMetrics;
      meeting.sentimentTimeline = sentimentTimeline;
      meeting.decisions = decisions;
      meeting.conflicts = conflicts;
      meeting.meetingCost = meetingCost;
      meeting.companyMemoryIndexed = true; // Mark for full-text search

      // Note: Voice summary URL would be generated by TTS service in production
      // For now, we store the text for future TTS generation
      // meeting.voiceSummaryUrl = await generateVoiceAudio(voiceSummaryText);
    } catch (aiError) {
      console.error('[MeetingController] AI feature generation error:', aiError);
      // Continue even if AI features fail - don't block the meeting processing
    }
...
// Company Memory Search - Search across all meetings in the team
export const searchMeetings = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (!query || query.trim().length < 2) {
      res.status(400).json({ success: false, error: { message: 'Search query must be at least 2 characters', code: 'INVALID_QUERY' } });
      return;
    }

    const limit = Math.min(20, Math.max(1, parseInt(req.query.limit as string) || 10));

    // Use MongoDB text search across transcript, summary, and title
    const searchResults = await Meeting.find(
      {
        team: activeTeam,
        $text: { $search: query },
        status: 'completed',
    ...
    // Also search in action items
    const actionItemResults = await ActionItem.find({
      team: activeTeam,
      $or: [
    ...
    res.json({
      ...
      data: {
        ...
        query,
        totalMeetings: searchResults.length,
        totalActionItems: actionItemResults.length,
      },
    });
  } catch (error: any) {
    res.status(500).json({ success: false, error: { message: error.message, code: 'SEARCH_FAILED' } });
  }
};
